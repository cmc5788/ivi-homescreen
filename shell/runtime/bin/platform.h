...
class Platform {
 public:
  // Perform runtime specific initialization.
  ...
  // format of the string is determined by the runtime. The returned string
...
#error Architecture detection failed.
  ...
  static const char* GetResolvedExecutableName() {
    if (resolved_executable_name_.load() == nullptr) {
      // Try to resolve the executable path using runtime specific APIs.
